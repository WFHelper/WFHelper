...
describe("settingsCategory", () => {
  ...
  it("lists the categories in sidebar order", async () => {
    const { prefs } = await loadPreferences();
    expect([...prefs.SETTINGS_CATEGORIES]).toEqual([
      "general",
      "notifications",
      "inventory",
      "overlay",
  ...
  it.each(["customization", "", "Overlay", "overlays"])(
    "falls back to General for the unknown value %j",
    async (raw) => {
      const { prefs } = await loadPreferences({ wf_settings_category: raw });
      expect(get(prefs.settingsCategory)).toBe("general");
    },
  );

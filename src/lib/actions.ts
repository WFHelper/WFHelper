...
export async function onInventoryLoaded(data: RawInventoryData): Promise<void> {
  ...
  if (parsedData?.InventoryJson && !parsedData?.Suits) {
    parsedData = parsedData.InventoryJson as RawInventoryData;
    if (typeof parsedData === "string") {
      try {
        parsedData = JSON.parse(parsedData) as RawInventoryData;
      } catch (error) {
        console.error("[actions] Failed to parse InventoryJson string:", error);
      }
    }
  }

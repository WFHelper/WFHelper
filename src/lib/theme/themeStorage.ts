import type { ThemeSettings, ThemeFontSizes } from "../../types/theme.js";
...
function migrateAndNormalize(raw: Record<string, unknown>): ThemeSettings {
  ...
  if (version < CURRENT_VERSION) {
    // Treat as fresh — merge with defaults
  }
  ...
  return {
    ...
    branding: {
      logoDataUrl:
        typeof rawBranding.logoDataUrl === "string" && rawBranding.logoDataUrl.startsWith("data:image/")
    ...
    contrastSafeMode:
      typeof raw.contrastSafeMode === "boolean" ? raw.contrastSafeMode : false,
  };
}

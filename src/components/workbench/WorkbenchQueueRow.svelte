<script lang="ts">
  ...
  const WARNING_KEYS: Record<WorkbenchQueueWarning, MessageKey> = {
    ...
    "override-needed": "workbench.warning.override-needed",
  ...
  const reservationTitle = $derived(
    row.verdict.reservations
      .map((reservation) => {
        ...
        const label = t(reservation.reasonKey, count != null ? { count } : {});
...
>
  ...
  <div class="min-w-0">
    ...
    {#if row.existingOrder}
      <span class="ml-2 text-xs opacity-70">
        {t("workbench.row.listed", { price: row.existingOrder.platinum })}
      </span>
    {/if}
    {#if warnings.length > 0}
      <div class="text-xs text-warning">
        {warnings.map((warning) => t(WARNING_KEYS[warning])).join(" · ")}
      </div>
    {/if}
  </div>
  ...
  <div class="text-right" title={reservationTitle}>
    ...
    {#if row.verdict.reserved > 0}
      ...
      >
        {t("workbench.row.reserved", { count: row.verdict.reserved })}
      </span>
    {/if}
  </div>
  ...
  <div class="text-right text-xs opacity-80">
    {#if row.market}
      <div>{t("workbench.row.lowest", { price: row.market.lowestSell ?? 0 })}</div>
      <div>
        {t("workbench.row.liquidity", { count: row.market.activeSellers })}
        {#if row.market.spread != null}
          · {t("workbench.row.spread", { spread: row.market.spread })}
        {/if}
      </div>
    {:else}
      <span class="opacity-50">{t("workbench.row.noMarketData")}</span>
    {/if}
  </div>
    ...
    {#if row.suggestion?.price != null}
      <span class="text-xs opacity-70" title={JSON.stringify(row.suggestion.inputs)}>
        ...
        {#if row.suggestion.damping}
          <span class="text-warning">{t(DAMPING_KEYS[row.suggestion.damping.reason])}</span>
        {/if}
      </span>
    {/if}
    <input
  ...
  <div class="flex items-center gap-1 text-xs">
    <button
      ...
      class="rounded-[var(--radius-sm)] border border-border px-2 py-1 {isLocked
        ...
        : 'bg-surface-hover'}"
      title={t("workbench.safety.lockHint")}
      onclick={() => onToggleLock(row)}
    >
      {t(isLocked ? "workbench.safety.locked" : "workbench.safety.lock")}
    </button>
    ...
    <input
      ...
      type="number"
      min="0"
      aria-label={t("workbench.safety.sparePlaceholder")}
      title={t("workbench.safety.spareHint")}
      value={spareOverride ?? ""}
    ...
    {#if canSetKeep}
      <button
        ...
        class="rounded-[var(--radius-sm)] border border-border px-2 py-1 {isSetKept
          ...
          : 'bg-surface-hover'}"
        title={t("workbench.safety.setKeepHint")}
        onclick={() => onToggleSetKeep(row)}
      >
        {t("workbench.safety.setKeep")}
      </button>
    {/if}
  </div>
</div>

<script lang="ts">
  ...
  export let backendHitOk = 0;
  export let backendHitNoData = 0;
...
<div class="mb-2.5 flex flex-wrap gap-[0.45rem] rounded-[0.42rem] border border-dashed border-blue-400/40 bg-blue-400/[0.08] px-[0.55rem] py-1.5 font-mono text-[0.72rem] leading-tight text-[#a8c9ff]">
  ...
  <span>queued={debug.queued}</span>
  <span>http={debug.priceDebugCounters.httpCalls}</span>
  <span>cacheOk={debug.priceDebugCounters.cacheHitOk}</span>

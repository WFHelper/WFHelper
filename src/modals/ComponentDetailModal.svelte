  ...
  <div class="detail-overlay comp-overlay" on:click|self={close}>
    ...
    <div class="detail-panel comp-panel">
      <div class="detail-panel-top-actions">
        <button class="detail-wiki-btn" on:click={() => ipc.openExternal(compWikiUrl ? toOfficialWikiUrl(compWikiUrl) : buildWikiUrl(parentName ? `${parentName} ${comp.name}` : comp.name))} title="Open on Wiki">
